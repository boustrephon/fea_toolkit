...
| `view_model.py` | **Unified viewer** — view a model or a saved results archive. Given a `.s2k` it parses, builds, runs the requested analysis (`mesh`, `static`, `modal`, `rs`, `pushover`, `interactive`) and displays it; given a `.npz` it displays the archived results directly, with no solver run. | `.s2k` / `.npz` / `--sample` |
