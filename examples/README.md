...
| `view_model.py` | **Unified viewer** — view a model or a saved results archive. Given a `.s2k` it parses, builds, runs the requested analysis (`mesh`, `static`, `modal`, `rs`, `pushover`, `interactive`) and displays it; given a `.npz` it displays the archived results directly, with no solver run. The `mesh` view also accepts `--zlim LO HI` (storey band), `--labels` (frame IDs), `--node-labels` (node IDs), `--highlight-section NAME...` (red vs. dimmed), `--highlight-constraint NAME...` (the joints of a SAP2000 constraint group — BODY, DIAPHRAGM, EQUAL, … — drawn red) and `--select "KEY=VALUE..."` (overdraw a `Selection` in yellow: wide lines over frames, dots over nodes; keys `type`, `section`, `material`, `group`, `constraint`, `id`, `z` — so `--select "constraint=Fix"` selects those same joints through the general Selection path). | `.s2k` / `.npz` / `--sample` |
