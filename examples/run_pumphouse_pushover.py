...
* ``examples/output/project_a_pushover_curve.png`` / ``.svg`` — capacity curve.
* ``examples/output/project_a_pushover_deformed.png`` / ``.svg`` — 3D deformed
  shape at peak displacement.
* ``examples/output/project_a_pushover_mode0.png`` / ``.svg`` …
  ``examples/output/project_a_pushover_mode2.png`` / ``.svg`` — first three mode
  shapes.

Usage::

    python examples/run_project_a_pushover.py /path/to/model.s2k
"""
...
def main():
    ...
    for m in range(min(3, n_modes)):
        ...
        if pl_png is not None:
            pl_png.screenshot(str(out / f"project_a_pushover_mode{m}.png"), scale=2)
            pl_png.close()
            print(f"  Saved → {out / f'project_a_pushover_mode{m}.png'}  (2×, 22pt)")
        ...
        if pl_svg is not None:
            try:
                pl_svg.save_graphic(str(out / f"project_a_pushover_mode{m}.svg"), raster=False)
                print(f"  Saved → {out / f'project_a_pushover_mode{m}.svg'}  (22pt, 3840×2880)")
            except Exception:
    ...
    fig = plot_pushover_curve(
        results,
        title='Project A — Non-linear Pushover (DEAD + SDL + Uniform)',
    )
    if fig is not None:
        fig.savefig(out / 'project_a_pushover_curve.png', dpi=300)
        print(f"\n  Saved → {out / 'project_a_pushover_curve.png'}  (300 dpi)")
        fig.savefig(out / 'project_a_pushover_curve.svg')
        print(f"  Saved → {out / 'project_a_pushover_curve.svg'}")
    ...
    try:
        ...
        _render_deformed(p, corner_font=22, label_font=22,
                         png_path=str(out / 'project_a_pushover_deformed.png'), png_scale=2)
        p.close()
        print(f"  Saved → {out / 'project_a_pushover_deformed.png'}  (2×, 22pt)")
        ...
        _render_deformed(s, corner_font=15, label_font=22,
                         svg_path=str(out / 'project_a_pushover_deformed.svg'))
        s.close()
        print(f"  Saved → {out / 'project_a_pushover_deformed.svg'}  (corner=15pt, labels=22pt, 3840×2880)")
