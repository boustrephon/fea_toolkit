...
parser = SAP2000Parser("project_b.s2k")
parser.parse()
md = parser.get_model_data()
```

**Key model-fix steps** (from `project_b_linear.py`):
...
with open("/tmp/project_b_po.tcl", "w") as f:
...
ret, stdout = runner.run("/tmp/project_b_po.tcl", timeout=600.0)
```
