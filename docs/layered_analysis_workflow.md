...
Private project-specific drivers, such as those under [local/<private_project>/Project A](local/<private_project>/Project A), should remain thin wrappers.
...
## 12. Impact on the Project B v3 workflow

The Project B v3 workflow should remain a “parse once, preprocess once, reuse many analyses” flow.
...
### 12.2 What this change means for the Project B v3 script

The Project B v3 script should not become a geometry builder per analysis case. That would reintroduce the old single-stage inefficiency.
...
For the Project B workflow, the likely practical differences are:
...
### 12.3 Practical effect on Project B outputs

Because the `MeshModel` remains the stable object, the Project B v3 output pipeline can keep producing the same downstream data products:
...
The main architectural change is that the Project B script will now explicitly separate:

- the shared prepared topology (`MeshModel`)
- the per-analysis realization (`AnalysisBuilder` + `AnalysisCaseSpec`)

## 13. Impact on the Project A v3 reporting workflow

The Project A v3 report workflow should follow the same pattern, but with a stronger emphasis on result reuse and case orchestration.

### 13.1 Intended runtime sequence

The Project A v3 flow should be structured as:
...
That means the generalised “run all” flow should be implemented once in the repository and then reused from private local drivers.  The local Project A wrapper should not duplicate the engine; it should delegate to the shared report entry point.
...
The Project A v3 report layer should be updated so that it:
...
That means the Project A v3 report continues to be a reporting front end, but the actual analysis pipeline moves to the cleaner shared-model / per-analysis-realization model.
...
- pushover analysis for the Project A case
...
TODO: add a concrete `AnalysisBuilder` nonlinear case entry point for the Project A pushover path where the current v3 report script needs a reusable nonlinear case object.
...
- use [local/<private_project>/Project_B/project_b_linear_v3.py](local/<private_project>/Project_B/project_b_linear_v3.py) as the canonical reference
