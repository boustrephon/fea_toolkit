...
**Fix:** `compute_lateral_loads()` now accepts an optional `nodal_masses` dict. The Project B script passes `nodal_masses=masses` from `b.compute_seismic_masses()` converted to `{node_tag: mass}`.
...
When `export_shells=False`, the gravity and lateral load dicts MUST be filtered to only include frame-connected nodes. This filtering is currently done in the Project B/validation script (external to the library), not in `builder.py`.
