...
| **1 — Rigid offset** | Rigid link from node to column face; flexure starts at face. | ✅ Implemented — explicit ``frame_end_offsets`` (stiff elastic links) or auto-generated via ``rigid_end_zones`` (offset = 0.5 × intersecting depth); ``rigid_link_mpc=True`` selects MPC links, otherwise stiff elastic links |
...
and Level 3 never double-count the same region.  ``rigid_link_mpc=True`` (optional) selects
``ops.rigidLink`` MPCs; otherwise stiff elastic elements are emitted —
the elastic links ill-condition the system under PDelta and can fail at
the gravity stage.  Level 2 would replace the rigid segments with
...
| ``run_all()`` (example orchestration) | ``local/…/project_a_report_v2.py`` | ⚠️ Project-specific, not generalised |
...
| **P2** | Auto line constraints | 3.1/3.2 | Medium (builder change) | Mesh-density transitions (wall ↔ slab, frame ↔ slab) need automatic detection and MPC application. Implemented as `find_constraint_edges()` — sorted-tuple edge registry + sweep-line chain following. Returns 46 edges for the Project B Building. | ✅ Done |
...
| **R1** | Generalised orchestrator `generate_report()` | 5 | Medium | Created ``fea_toolkit/report.generate_report(md, mesh_model, config, out_dir, **overrides) → dict``.  Extracts the generic pipeline orchestration from ``project_a_report_v2.run_all()``.  The project-specific script is now a thin wrapper. | ✅ Done |
...
| **R3** | HPC job submission + result ingest | — | Large | Helper to submit Tcl to Xara (OpenSeesMP), wait for completion, parse output back into the unified NPZ/HDF5 schema.  Required for `project_b_nonlinear.py`. | ❌ Pending |
...
The existing `local/` reports (e.g. `project_b_report.qmd`, `project_a_report_v2.qmd`)
contain inline Python cells that combine computation and presentation.
The migration strategy:

| Step | What changes | Status |
|---|---|---|
| **1. Adopt Python-dict config** | The proposed YAML config was replaced by a Python ``_DEFAULT_CONFIG`` dict in each report script.  This is more flexible for programmatic use and avoids a YAML dependency. | ✅ Done (see ``project_a_report_v2.py``) |
...
| **3. Generalise orchestration** | Extract pipeline orchestration from ``project_a_report_v2.run_all()`` into ``fea_toolkit/report.py`` as ``generate_report()``. | ✅ Done (R1) |
| **4. Refactor QMD** | Replace inline computation cells with HDF5 read + plot/table cells. | ⚠️ Partially done (``project_a_report_v2.qmd`` reads cached results) |
