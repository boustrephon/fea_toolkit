...
| **P2** | Auto edge constraints | 3.1/3.2 | Medium (builder change) | Mesh-density transitions (wall ↔ slab) need automatic detection and MPC application. Implemented as `find_constraint_edges()` — sorted-tuple edge registry + sweep-line chain following. Returns 46 edges for the Project B Building. | ✅ Done |
