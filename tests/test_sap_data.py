...
class TestTrapezoidalForceSplit:
    ...
    def test_split_at_midpoint(self):
        ...
        assert len(result) == 2
        # Both halves should still have force=10 at both ends
        for seg in result:
            assert seg[0][1] == pytest.approx(10.0)
            assert seg[1][1] == pytest.approx(10.0)
...
class TestConcreteRectangularSectionFiberPatches:
    ...
    def test_concrete_rect_no_rebar(self):
        ...
        for p in patches:
            assert p[0] == "rect"
        assert len(patches) == 5  # 1 confined core + 4 unconfined covers
