...
def test_spectrum_forks_into_two_composites():
    ...
    assert out["SEISM [+RSX]"]["nodal_displacements"][1] == pytest.approx([0.02, 0.0, -0.01])
    assert out["SEISM [-RSX]"]["nodal_displacements"][1] == pytest.approx([-0.02, 0.0, -0.01])
...
def test_default_expands_every_combination():
    ...
    assert {"GRAV", "SEISM [+RSX]", "SEISM [-RSX]", "ENV [max]", "ENV [min]"} <= set(out)
...
def test_export_results_expands_combinations(monkeypatch, tmp_path):
    ...
    assert {"GRAV", "SEISM [+RSX]", "SEISM [-RSX]", "ENV [max]", "ENV [min]"} <= set(merged)
    ...
    assert captured["case_meta"]["SEISM [+RSX]"]["coords"] == "+RSX"
    assert captured["case_meta"]["SEISM [-RSX]"]["coords"] == "-RSX"
    assert captured["case_meta"]["SEISM [+RSX]"]["group"] == "SEISM"
    assert captured["case_meta"]["SEISM [+RSX]"]["family"] == "fork"
...
def test_export_results_expands_external_definitions(monkeypatch, tmp_path):
    ...
    merged = captured["static_results"]
    assert {"EXT [+RSX]", "EXT [-RSX]"} <= set(merged)
    assert captured["case_meta"]["EXT [+RSX]"]["coords"] == "+RSX"
    assert captured["case_meta"]["EXT [-RSX]"]["coords"] == "-RSX"
...
def test_definitions_expand_without_a_model():
    ...
    assert set(out) == {"EXT [+RSX]", "EXT [-RSX]"}
    assert out["EXT [+RSX]"]["element_forces"]["fx_i"] == pytest.approx([25.0, 45.0])
    # The −RSX sense subtracts the magnitude again: 2·DEAD − RSX.
    assert out["EXT [-RSX]"]["element_forces"]["fx_i"] == pytest.approx([15.0, 35.0])
...
def test_return_meta_describes_every_case():
    ...
    assert meta["DEAD"] == {"group": "DEAD", "family": "single", "coords": ""}
    assert meta["SEISM [+RSX]"] == {
        "group": "SEISM",
        "family": "fork",
        "coords": "+RSX",
    }
