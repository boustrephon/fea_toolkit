...
| Array | Shape | dtype | Description |
|---|---|---|---|
| `schema_version` | `()` | `int` | File-level results-layout version (`SCHEMA_VERSION`, currently 3); absent in legacy files → read as 1 |
...
SAP2000Parser ──→ SAPModelData
    ...
    │               (dict, NPZ or HDF5)  (plot_mesh, plot_force_diagram, …)
    │
    └── plot_force_diagram() ──→ matplotlib 2D / PyVista 3D (path, dict or NPZ)
...
See also the `local/project_b_linear.py` script for a complete workflow
