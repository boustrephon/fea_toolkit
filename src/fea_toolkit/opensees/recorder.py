    ...
    def save_as_xara_tcl(
        ...
        if not lib_path:
            try:
                ...
                candidate = os.path.join(lib_dir, "libOpenSeesRT.dylib")
                if os.path.exists(candidate):
                    lib_path = candidate
                else:
                    candidate = os.path.join(lib_dir, "libOpenSeesRT.so")
                    if os.path.exists(candidate):
                        lib_path = candidate
            except ImportError:
                lib_path = "libOpenSeesRT.dylib"  # hope it's on the path
        ...
        # Commands filtered: skip query calls and our own preamble
        _skip = {
            "wipe", "model", "wipeAnalysis",
            # OpenSeesPy query calls (no Tcl model-building equivalent)
            "nodeCoord", "getNodeTags", "getEleTags", "eleNodes",
            "nodeDisp", "nodeEigenvector", "nodeReaction", "nodeMass",
            "eleResponse", "modalProperties", "loadConst",
            "reactions", "responseSpectrumAnalysis",
            "eigen", "analyze",
            # Analysis/solver setup (emitted separately)
...
class XaraTclRunner:
    ...
    def which_tclsh() -> str:
        ...
        for candidate in [
            "/Users/<user>/miniforge3/bin/tclsh8.6",
