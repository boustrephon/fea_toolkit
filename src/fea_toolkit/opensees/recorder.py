...
def parse_pushover_results(
    ...
    Reads the output files generated by :func:`pushover_tcl` (whose
    recorder filenames are controlled by its ``output_prefix``
    parameter, default ``"wall"``):
    ...
    dict
        ...
        - ``"reaction_rx"``, ``"reaction_ry"``, ``"reaction_rz"`` —
          per-step reaction channels from *reaction_path* (only when
          the file has the corresponding columns).
